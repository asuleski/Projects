...
subtitle: "An Application of Time Series Methods and Integration of Functional Modeling"
author: "Amanda Suleski, Evan Tiffany, Ann Marie Matheny"
date: "4/1/2020"
...
knitr::kable(missing, caption = "Missing Data") %>% kable_styling(position = "center", latex_options = c("hold_position"))
```
...
```{r fig.align='center', fig.show = 'hold', fig.width=4, fig.height=3}
...
components <- decompose(series, type = "additive")
plot(components, cex.lab = 0.6)
...
ggplot(data = sst.dat, aes(x = as.numeric(DATE), y = SST)) + 
  ...
  ggtitle("Linear Model") + xlab("Date") + ylab("Temperature") + theme_light()
...
ggplot(data = fore.dat, aes(x = DATE, y = SST)) + 
  ...
  ylab("Temperature") + ggtitle("Linear Model Forecast") + theme_light()
...
$$\hat{X_t}(k) \pm Z_{\alpha /2} \sqrt{Var(e_t(k))}$$
where $Z_{\alpha /2}$ is the one-tailed value of the standard normal distribution. For a more in depth analysis visit “Prediction Intervals for the Holt-Winters Forecasting Procedure” by Chatfield and Yar.
...
ggplot() + geom_line(aes(x = 1:61, y = as.numeric(fc)), col = "blue") + 
  geom_line(aes(x = 1:61, y = all.dat$SST[781:841]), col = "red") + 
  xlab("Date") + ylab("Temperature") + ggtitle("Holt-Winters Model Fit") +
  theme_light()
```
...
ggplot(data = fore.dat, aes(x = DATE, y = SST)) + 
  ...
  ylab("Temperature") + ggtitle("Functional Model Forecast") + theme_light()
